...
const existingAdmin = db.prepare("SELECT id FROM AdminUser WHERE email = ?").get("admin@aalb.org");
if (!existingAdmin) {
  db.prepare(
    "INSERT INTO AdminUser (id, email, password, name, createdAt) VALUES (?, ?, ?, ?, ?)"
  ).run(adminId, "admin@aalb.org", hashedPassword, "AALB Admin", new Date().toISOString());
}
...
const jobs = [
  ...
  },
  {
    ...
    description: "AALB is looking for a creative and strategic Marketing & Communications Manager to lead our brand presence and member communications. You will develop and execute marketing campaigns, manage our digital presence, and create compelling content that advances our mission.\n\nThis role is ideal for someone who is passionate about storytelling and has experience in the financial services or association space.",
  ...
  },
  {
    ...
    description: "Join our technology team to build and maintain the digital platforms that power AALB's member services, educational programs, and industry data analytics tools.\n\nYou will work on a modern tech stack and have the opportunity to make a direct impact on how lending professionals access resources and connect with each other.",
  ...
  },
  {
    ...
    description: "We are looking for a detail-oriented Member Services Coordinator to serve as the primary point of contact for our member organizations. You will manage member inquiries, coordinate events, and ensure an excellent member experience.\n\nThis is a great opportunity for someone early in their career who wants to grow within a mission-driven organization.",
  ...
  },
  {
    ...
    description: "AALB's Research & Data team is seeking a Data Analyst to support our industry benchmarking and market research initiatives. You will analyze lending industry trends, produce reports for our members, and contribute to AALB's reputation as a leading source of industry intelligence.",
    requirements: "Bachelor's degree in Statistics, Economics, Data Science, or related field\n2+ years of data analysis experience\nProficiency in SQL, Python or R\nExperience with data visualization tools (Tableau, Power BI)\nStrong analytical and critical thinking skills\nExcellent report writing abilities\nExperience in financial services or lending industry a plus",
  ...
  },
  {
    ...
    type: "Part-time",
    salary: "$20/hour",
    description: "AALB is seeking an enthusiastic Events & Conference Intern to support the planning and execution of our annual conference and regional events. This is a paid internship ideal for students or recent graduates interested in event management and the association industry.",
    requirements: "Currently enrolled in or recently graduated from a Bachelor's program\nStrong organizational skills and attention to detail\nAbility to work in a fast-paced environment\nExcellent communication skills\nProficiency in Google Workspace or Microsoft Office\nAvailability to work some evenings and weekends during events",
    benefits: "Paid internship ($20/hour)\nFlexible schedule around classes\nNetworking opportunities with industry leaders\nPotential for full-time offer upon completion",
  },
];

const now = new Date().toISOString();
const insertJob = db.prepare(
  `INSERT INTO Job (id, title, department, location, type, salary, description, requirements, benefits, isActive, createdAt, updatedAt)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, ?)`
);
...
console.log("Created 6 sample jobs and admin user.");
