    ...
    <div className="min-h-screen bg-sand-50 lg:flex">
      ...
      <div className="min-w-0 flex-1">
        <main className="mx-auto max-w-5xl px-5 py-8 sm:px-8 lg:px-12 lg:py-12">
          <div className="mb-2 h-7 w-44 animate-pulse rounded-lg bg-sand-200" />
          <div className="mb-8 h-4 w-80 animate-pulse rounded bg-sand-200/70" />
          <div className="mb-10 grid gap-4 sm:grid-cols-3">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="h-28 animate-pulse rounded-2xl border border-sand-200/80 bg-white"
              />
            ))}
          </div>
          <div className="h-72 animate-pulse rounded-2xl border border-sand-200/80 bg-white" />
        </main>
      </div>
    </div>
  );
